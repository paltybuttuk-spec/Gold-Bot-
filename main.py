...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/levels — Key S/R levels map + DXY correlation\n"
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"  Fib lookback:        `{cfg.fib_lookback_candles} candles`\n"
        f"  DXY corr threshold:  `{cfg.dxy_correlation_pct*100:.1f}%`\n"
        f"━━━━━━━━━━━━━━━━━━━━",
        parse_mode="Markdown",
    )
...
async def cmd_chart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIX: send photo FIRST, then delete placeholder
    # If send_photo fails the user still sees the error via the placeholder.
    await update.message.reply_photo(photo=buf, caption=caption)
    try:
        await msg.delete()
    except Exception:
        pass  # placeholder already gone — not fatal
